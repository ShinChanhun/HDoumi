namespace Doumi.Bot
{
    ...
    public class Hunt_Wizard
    {
        ...
        int[] monsterArr = { 16586, 16701, 16590, 16699 };

        public Hunt_Wizard(ProxyPatron patron)
        {
            this.Patron = patron;
        }
        ...
        bool CheckMonster(ushort icon)
        {
            return false;
            foreach (ushort arrIcon in monsterArr)
            {
                if (arrIcon == icon) return true;
            }
                ...
                if (((double)this.Patron.CurrentMP.Get()) > (((double)this.Patron.MaximumMP.Get()) * 0.1))
                {
                    ...
                    if (monster == null)
                    {
                        foreach (KeyValuePair<uint, Monster> pair in field.Monsters)
                        {
                            ...
                            {
                                //if (!CheckMonster(pair.Value.Icon))
                                //{
                                //    continue;
                                //}
                                flag2 = true;
                            }
                            if (((this.FirstMonsters(pair.Value.Icon) && (pair.Value.Tint == 0)) && (num7 < num5)) && this.Patron.Form.isAttackable(pair.Value.X, pair.Value.Y))
                            {
                                //if (!CheckMonster(pair.Value.Icon))
                                //{
                                //    continue;
                        ...
                        if (!flag2)
                        {
                            foreach (KeyValuePair<uint, Monster> pair2 in field.Monsters)
                            {
                                ...
                                if (((!this.IgnoreMonsters(pair2.Value.Icon) && (pair2.Value.Tint == 0)) && (num8 < num5)) && this.Patron.Form.isAttackable(pair2.Value.X, pair2.Value.Y))
                                {
                                    //if (!CheckMonster(pair2.Value.Icon))
                                    //{
                                    //    continue;
                                    //}
                                    num5 = num8;
                    ...
                    if (((num5 != 0x7fffffff) && (num5 < 9)) && (monster2 != null))
                    {
                        ...
                        if ((field.Name.StartsWith("블랙오피온의굴8") && (monster2.X == 0x15)) && (monster2.Y == 12) /*&& !CheckMonster(monster2.Icon)*/)
                        {
                            Monster monster4;
                        ...
                        if (flag15)
                        {
                            ...
                            if (!monster2.Immortal)
                            {
                                ...
                                this.Patron.UseSpell("마레노", monster2);
                            }
                        }
                    }
                    else
                    {
                        ...
                        foreach (KeyValuePair<uint, Monster> pair3 in field.Monsters)
                        {
                            ...
                            if (num14 <= 8 /*&& !CheckMonster(pair3.Value.Icon)*/)
                            {
                                num13++;
                            }
                            else
